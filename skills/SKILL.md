...
`oled --version` prints a version when installed. To install: check `node --version` >= 18, then `npm install -g open-ledger`. First run: `oled config --generate-key --json` unless `oled status --json` shows `"configured":true`. Statements go in the `dataDir` from `oled config show --json`, as PDFs or as photos/scans (PNG/JPEG/WebP); `oled open` opens it.
