...
2. **Prepares** it — the vault decrypts the PDF and returns a readable
   `document` path (`oled ingest prepare`).
...
5. **Reports** back — spending by category, refunds, top merchants — from
   `oled status`, `oled report`, and `oled transactions list`.

The agent may run only `oled`, read/write files, and use Claude Code
Skills (`--allowedTools "Bash(oled:*),Read,Write,Skill"`).
...
- `npm start -- --skip-claude` — skip the live `claude -p` turns and only
  ...
  unlock, and that `oled ingest list` discovers the statement). Useful
...
the answer markdown parser; it needs neither `claude` nor the OpenLedger build.
...
3. **write bin shim** — an `oled` shim script pointing at the freshly
   built `dist/cli/index.js`.
4. **export isolation env** — `PATH` (so the shim resolves), `HOME` /
   `USERPROFILE`, and the `OLED_*` variables described below.
5. **place statement** — copies `fixtures/card-statement-2026-05.pdf` into the
   workspace's data directory.
6. **install skill** — installs the OpenLedger skill pack so `claude` can
...
`oled ingest list --json` and asserts at least one newly-discovered file
...
own panel showing the tool calls the agent makes (`> oled ...`,
...
check) whether the agent loaded the OpenLedger skill, how many `oled`
...
## Isolation
The run never touches your real `~/.oled`. Every run builds a 
fresh, isolated workspace and redirects `HOME`/`USERPROFILE`,
`OLED_DIR`, `OLED_DB_PATH`, `OLED_DATA_DIR`, and 
`OLED_CACHE_DIR` into it before doing anything else — the run never 
reads or writes your real `~/.oled` installation (if you have one). 
